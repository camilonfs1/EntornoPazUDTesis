...
import com.example.entornopazud.view.QualiModule.statisticsTeacher
import kotlinx.android.synthetic.main.recycler_row.view.*

class Adapter_Students (items: ArrayList<User>, resourcek : Int, mUser : String,activity:String) : RecyclerView.Adapter<Adapter_Students.ViewHolder>(){
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {//information about each item of the holder
        ...
        }else if(activit=="Statistic"){
            holder.vista.setOnClickListener(object : View.OnClickListener  {
                override fun onClick(v: View?) {
                    var intent = Intent(v!!.context,statisticsTeacher::class.java)
                    intent.putExtra("key",item?.key)
                    intent.putExtra("name",item?.name)
                    intent.putExtra("id",item?.id)
                    intent.putExtra("email",item?.email)
